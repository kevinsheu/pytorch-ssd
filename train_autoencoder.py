...
for i, data, in enumerate(train_dataloader):
    ...
    total_loss = loss_fn(curr_embed, same_embed, torch.tensor([1]).to(device))
    ...
    if i % (batch_size * 7) == 0:
        ...
        with torch.no_grad():
            ...
            for test_i, test_data in enumerate(test_dataloader):
                ...
                same_dist = distance_fn(curr_embed, same_embed).to('cpu').numpy()[0]
                diff_dist = distance_fn(curr_embed, diff_embed).to('cpu').numpy()[0]
